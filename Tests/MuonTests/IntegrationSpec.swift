        ...
        describe("Sparkfun") {
            ...
            it("should parse the feed") {
                ...
                if let feed = feed {
                    ...
                    expect(feed.link).to(equal(URL(string: "https://www.sparkfun.com/news")!))
                }
            }
        ...
        describe("RSS 0.92") {
            ...
            it("should parse the feed") {
                ...
                if let feed = feed {
                    ...
                    expect(feed.link).to(equal(URL(string: "http://www.scripting.com/blog/categories/gratefulDead.html")))
            ...
            it("should parse the feed's items") {
                ...
                if let article = feed?.articles.last {
                    ...
                    if let enclosure = article.enclosures.first {
                        expect(enclosure.url).to(equal(URL(string: "http://www.scripting.com/mp3s/youWinAgain.mp3")))
        ...
        describe("RSS 1.0") {
            ...
            it("should parse the feed") {
                ...
                if let feed = feed {
                    ...
                    expect(feed.link).to(equal(URL(string: "http://xml.com/pub")))
                    expect(feed.description).to(match("XML.com features a rich mix of information and services for the XML community."))
                    expect(feed.imageURL).to(equal(URL(string: "http://xml.com/universal/images/xml_tiny.gif")))
                }
            }
        ...
        describe("Atom 1.0") {
            ...
            it("should parse the feed") {
                ...
                if let feed = feed {
                    ...
                    expect(feed.link).to(equal(URL(string: "http://example.org/feed.atom")))
                    let date = "2005-07-31T12:29:29Z".RFC3339Date()
                    expect(feed.lastUpdated).to(equal(date))
                    expect(feed.copyright).to(equal("Copyright (c) 2003, Mark Pilgrim"))
                    expect(feed.imageURL).to(equal(URL(string: "http://example.org/icon.gif")))
                }
            }
            ...
            it("should parse the feed's items") {
                ...
                if let article = feed?.articles.first {
                    ...
                    if let author = article.authors.first {
                        ...
                        expect(author.email).to(equal(URL(string: "f8dy@example.com")))
                        expect(author.uri).to(equal(URL(string: "http://example.org")))
                    }
